...
exports.publicTicketInvitation = async (req, res, next) => {
  try {
    ...
    const androidIntentLink = `intent://invite/${encodeURIComponent(req.params.shareToken)}#Intent;scheme=rtc-customer;package=com.ea.rtcuser;S.browser_fallback_url=${encodeURIComponent(androidPlayStoreURL)};end`;
    const iosAppStoreURL = server.customerIosAppStoreURL;
    const androidPlayStoreURL = server.customerAndroidPlayStoreURL;
    res.set({ 'Cache-Control': 'no-store, private', 'X-Content-Type-Options': 'nosniff' });
    return res.type('html').send(`<!doctype html><html><head><meta name="viewport" content="width=device-width,initial-scale=1"><title>RTC Event Invitation</title><style>body{font-family:-apple-system,sans-serif;background:#0f172a;color:#fff;padding:28px}.card{max-width:480px;margin:auto;background:#fff;color:#172033;padding:28px;border-radius:22px}.button{display:block;text-align:center;background:#ea580c;color:#fff;padding:16px;border-radius:14px;text-decoration:none;font-weight:800;border:0;width:100%;font-size:16px;cursor:pointer;box-sizing:border-box}.stores{display:flex;gap:10px;margin-top:10px}.store{flex:1;display:block;text-align:center;color:#c2410c;padding:14px 8px;text-decoration:underline;font-weight:700}</style></head><body><main class="card"><p>ROUND DA' CORNER</p><h1>${String(event.event_name).replace(/[<>&]/g, '')}</h1><p>${String(event.event_city || '')}, ${String(event.event_state || '')}</p><p>Open Round Da' Corner, sign in or create your customer profile, then select your tickets.</p><button class="button" type="button" onclick="openRtcApp()">Open Ticket Checkout</button><div class="stores"><a class="store" href="${iosAppStoreURL}">App Store</a><a class="store" href="${androidPlayStoreURL}">Google Play</a></div></main><script>function openRtcApp(){var userAgent=navigator.userAgent||'';var isIos=/iPad|iPhone|iPod/.test(userAgent);var isAndroid=/Android/.test(userAgent);if(isAndroid){window.location.href=${JSON.stringify(androidIntentLink)};return}if(isIos){var fallbackTimer=setTimeout(function(){if(document.visibilityState==='visible'){window.location.replace(${JSON.stringify(iosAppStoreURL)})}},1800);document.addEventListener('visibilitychange',function(){if(document.visibilityState==='hidden'){clearTimeout(fallbackTimer)}},{once:true});window.location.href=${JSON.stringify(deepLink)};return}window.location.href=${JSON.stringify(deepLink)}}</script></body></html>`);
  } catch (error) {
    return next(error);
  }
};
