...
exports.publicTicketInvitation = async (req, res, next) => {
  try {
    ...
    res.set({ 'Cache-Control': 'no-store, private', 'X-Content-Type-Options': 'nosniff' });
    return res.type('html').send(`<!doctype html><html><head><meta name="viewport" content="width=device-width,initial-scale=1"><meta name="apple-itunes-app" content="app-id=6748915125, app-argument=${invitationURL}"><title>RTC Event Invitation</title><style>body{font-family:-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif;background:#0f172a;color:#fff;padding:28px}.card{max-width:480px;margin:auto;background:#fff;color:#172033;padding:28px;border-radius:22px}.status{color:#526176;line-height:1.45}.store{display:block;text-align:center;background:#ea580c;color:#fff;padding:16px;border-radius:14px;text-decoration:none;font-weight:800;margin-top:12px}.secondary{background:#172033}.hidden{display:none}</style></head><body><main class="card"><p>ROUND DA' CORNER</p><h1>${String(event.event_name).replace(/[<>&]/g, '')}</h1><p>${String(event.event_city || '')}, ${String(event.event_state || '')}</p><p id="status" class="status">Opening the Round Da' Corner customer app…</p><a id="ios-store" class="store hidden" href="${iosAppStoreURL}">Download on the App Store</a><a id="android-store" class="store hidden" href="${androidPlayStoreURL}">Get it on Google Play</a><div id="both-stores" class="hidden"><a class="store" href="${iosAppStoreURL}">Download on the App Store</a><a class="store secondary" href="${androidPlayStoreURL}">Get it on Google Play</a></div><noscript><p class="status">Install the Round Da' Corner customer app, then tap the original event link again.</p><a class="store" href="${iosAppStoreURL}">Download on the App Store</a><a class="store secondary" href="${androidPlayStoreURL}">Get it on Google Play</a></noscript></main><script>(()=>{const ua=navigator.userAgent||'';const ios=/iPad|iPhone|iPod/.test(ua);const android=/Android/.test(ua);const status=document.getElementById('status');let target=null;if(ios){target=${JSON.stringify(iosAppStoreURL)};document.getElementById('ios-store').classList.remove('hidden')}else if(android){target=${JSON.stringify(androidPlayStoreURL)};document.getElementById('android-store').classList.remove('hidden')}else{document.getElementById('both-stores').classList.remove('hidden');status.textContent='Install the Round Da\' Corner customer app, then open the original event link again.'}if(target){status.textContent='If the app does not open, install it below and then tap the original event link again.';setTimeout(()=>window.location.assign(target),900)}})()</script></body></html>`);
  } catch (error) {
    return next(error);
  }
};
