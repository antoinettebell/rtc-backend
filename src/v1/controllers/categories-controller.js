const { CategoriesService: Service ,MenuCategoryService} = require('../services');
...
exports.add = async (req, res, next) => {
  try {
    ...
    if (existRecord) {
      return res.error(
        new Error('Categories with this name is already exists.'),
        409
      );
    }
...
exports.update = async (req, res, next) => {
  try {
    ...
    if (!item) {
      return res.error(new Error('No Categories found'), 409);
    }
...
exports.destroy = async (req, res, next) => {
  try {
   ...
   const counts = await Service.getCategoriesCount({ categoriesId: id });
    if (counts > 0) {
      return res.error(
         new Error('You cannot remove this category because a vendor has already added menus under it'),
        409
      );
    }
