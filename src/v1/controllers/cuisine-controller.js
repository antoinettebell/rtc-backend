const { CuisineService: Service } = require('../services');
...
exports.add = async (req, res, next) => {
  try {
    ...
    if (existRecord) {
      return res.error(
        new Error('Cuisine with this name is already exists.'),
        409
      );
    }
...
exports.update = async (req, res, next) => {
  try {
    ...
    if (existRecord) {
      return res.error(new Error('Cuisine with this name is already exists.'), 409);
    }
