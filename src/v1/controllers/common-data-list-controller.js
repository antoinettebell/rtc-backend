const { CommonDataListService: Service } = require('../services');
const entityName = 'listing';
...
exports.list = async (req, res, next) => {
  try {
    let {
      query: { limit = 10, page = 1, search ,type="meat_wellness"},
      params: { id: _id },
    } = req;
    if (_id) {
    ...
    if (search && search.trim()) {
      q.$or = [{ name: { $regex: search.trim().toLowerCase(), $options: 'i' } }];
    }
