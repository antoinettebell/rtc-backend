...
function timebox(action,milliseconds){return Promise.race([Promise.resolve().then(action).catch(()=>{}),new Promise(resolve=>setTimeout(resolve,milliseconds))])}
async function closeCamera(){const activeScanner=scanner;scanner=null;running=false;if(!activeScanner)return;await timebox(()=>activeScanner.stop(),1500);await timebox(()=>activeScanner.clear(),1500)}
...
async function scan(decoded){if(busy)return;busy=true;show('processing','Validating ticket…');if(navigator.vibrate)navigator.vibrate(100);const cameraClosing=closeCamera();try{const token=new URL(decoded).pathname.split('/').filter(Boolean).pop();if(!token)throw new Error('Ticket is invalid');const controller=new AbortController();const timeout=setTimeout(()=>controller.abort(),12000);let response;try{response=await fetch('/api/v1/public/marketplace/tickets/validate',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({event_id:eventId,scanner_session_token:session,ticket_token:token}),signal:controller.signal})}finally{clearTimeout(timeout)}const data=await response.json();if(!response.ok||!data.success)throw new Error(data.message||'Ticket is invalid');const value=data.data||data;if(navigator.vibrate)navigator.vibrate([100,50,100]);show('success','CHECKED IN: '+value.attendeeName+' ('+value.ticketType+')')}catch(error){if(navigator.vibrate)navigator.vibrate(400);show('error',error.name==='AbortError'?'Ticket validation timed out. Tap Scan Next Ticket to retry.':error.message)}finally{await cameraClosing;busy=false;button.textContent='Scan Next Ticket'}}
