...
async function scan(decoded){if(busy)return;busy=true;await stop();show('processing','Validating ticket…');if(navigator.vibrate)navigator.vibrate(100);try{const token=new URL(decoded).pathname.split('/').filter(Boolean).pop();const response=await fetch('/api/v1/public/marketplace/tickets/validate',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({event_id:eventId,scanner_session_token:session,ticket_token:token})});const data=await response.json();if(!response.ok||!data.success)throw new Error(data.message||'Ticket is invalid');const value=data.data||data;if(navigator.vibrate)navigator.vibrate([100,50,100]);show('success','CHECKED IN: '+value.attendeeName+' ('+value.ticketType+')')}catch(error){if(navigator.vibrate)navigator.vibrate(400);show('error',error.message)}setTimeout(()=>{busy=false;start()},2500)}
