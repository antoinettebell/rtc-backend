...
assert.ok(itemSchema.path('employee_modified_at'));
assert.ok(itemSchema.path('employee_modified_by_id'));
...
assert.match(service, /incoming\.employee_modified === true/);
assert.match(service, /filter\(\(item\) => item\.employee_modified_at\)/);
